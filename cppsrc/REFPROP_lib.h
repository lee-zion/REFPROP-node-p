    ...
    bool load_REFPROP(std::string &err, const std::string &shared_library_path, const std::string &shared_library_name)
    {
        ...
        if (RefpropdllInstance == NULL)
        {
            ...
            if (RefpropdllInstance == NULL)
            {
                ...
                // Could not load REFPROP (librefprop.dylib)
                // due to: dlopen(/Users/econ/ref-backend/src/refprop/librefprop.dylib, 2): image not found.
                // Make sure the library is in your system search path.
                // In case you run 64bit Windows and you have a REFPROP license,
